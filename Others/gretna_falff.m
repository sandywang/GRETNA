function gretna_falff(Data_path, File_filter, SamplePeriod, FreBand, Data_mask, Output_path)

%==========================================================================
% This function is used to fALFF in a voxelwise manner.
%
%
% Syntax: function gretna_falff(Data_path, File_filter, SamplePeriod, FreBand, Data_mask, Output_path)
%
% Inputs:
%       Data_path:
...
for i = 1:Num_subs
    ...
    fprintf('Calculating fALFF for %s\n', [Dir_data{1}{i}]);
    ...
    NFFT = gretna__nextpow2(Timepoints);
    ...
    fALFF = zeros(Rows,Columns,Slices);
    PSD = 2*abs(fft(Ydata, NFFT))/Timepoints;    
    fALFF(IndMask) = sum(PSD(IndFre,:))./sum(PSD(1:NFFT/2+1,:));
    
    cd (Output_path)
    Vout = Vin(i);
    [~, name, ~] = fileparts(Dir_data{1}{i});
    Vout.fname = ['fALFF_' name '.img'];
    Vout.dt(1) = 16;
    Vout = spm_write_vol(Vout, fALFF);
    
    fprintf('Calculating fALFF for %s ...... is done\n', [Dir_data{1}{i}]);
