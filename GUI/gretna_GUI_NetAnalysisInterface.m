...
% Init Postprocess Procedures
handles.NodalModeCell={...
    'Nodal - Clustering Coefficient',  1;...
    'Nodal - Shortest Path Length',    2;...
    'Nodal - Efficiency',              3;...
    'Nodal - Local Efficiency',        4;...    
    'Nodal - Degree Centrality',       5;...
    'Nodal - Betweenness Centrality',  6;...
    'Nodal - Community Index',         7;...
    'Nodal - Participant Coefficient', 8;...
    'Modular - Interaction',           9};

handles.Para.CIndex={'<-X'};
% Init Mode List Index
handles.UnselPreInd=(1:6)';
handles.SelPreInd=[];
handles.UnselPostInd=(1:9)';
handles.SelPostInd=[];
...
%             Pl=gretna_FUN_Cell2Pipe(Pl, TList, PCell);
        ...
        case 'NODAL - CLUSTERING COEFFICIENT'
            OutputMatList=cellfun(@(a) fullfile(OutputDir, a, 'NCP.mat'),...
                AList, 'UniformOutput', false);
            InterMat=GenInterMat(OutputDir, 'NodalClustCoeff', GrpID);
            PCell=cellfun(@(in, out)...
                gretna_GEN_NodalClustCoeff(in, out, Para.NetType{1}, AUCInterval),...
                RealNetList, OutputMatList,...
                'UniformOutput', false);
    
            TList=cellfun(@(i) sprintf('NodalClustCoeff%s', i), IndList,...
                'UniformOutput', false);
            Pl=gretna_FUN_Cell2Pipe(Pl, TList, PCell);             
        case 'NODAL - SHORTEST PATH LENGTH'
            OutputMatList=cellfun(@(a) fullfile(OutputDir, a, 'NLP.mat'),...
                AList, 'UniformOutput', false);
            InterMat=GenInterMat(OutputDir, 'NodalShortestPath', GrpID);
            PCell=cellfun(@(in, out)...
                gretna_GEN_NodalShortestPath(in, out, Para.NetType{1}, AUCInterval),...
                RealNetList, OutputMatList,...
                'UniformOutput', false);
    
            TList=cellfun(@(i) sprintf('NodalShortestPath%s', i), IndList,...
                'UniformOutput', false);
            Pl=gretna_FUN_Cell2Pipe(Pl, TList, PCell);              
        case 'NODAL - LOCAL EFFICIENCY'
            OutputMatList=cellfun(@(a) fullfile(OutputDir, a, 'NLE.mat'),...
                AList, 'UniformOutput', false);
            InterMat=GenInterMat(OutputDir, 'NodalLocalEfficiency', GrpID);
            PCell=cellfun(@(in, out)...
                gretna_GEN_NodalLocalEfficiency(in, out, Para.NetType{1}, AUCInterval),...
                RealNetList, OutputMatList,...
                'UniformOutput', false);
    
            TList=cellfun(@(i) sprintf('NodalLocalEfficiency%s', i), IndList,...
                'UniformOutput', false);
            Pl=gretna_FUN_Cell2Pipe(Pl, TList, PCell);            
