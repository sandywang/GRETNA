    ...
    for i=1:size(handles.DataList)
        ...
        if strcmp(Ext , '.txt')
            ...
            MatrixList=[MatrixList ;...
                {TempMat}];
            AliasList=[AliasList;{sprintf('%s_TXT', Name)}];
        elseif strcmp(Ext , '.mat')
            TempStruct=load([Path , filesep , Name , Ext]);
            FieldNames=fieldnames(TempStruct);
            for j=1:numel(FieldNames)
                ...
                if iscell(TempMat)
                    ...
                    for k=1:numel(TempMat)
                        AliasList=[AliasList;{sprintf('%s_VAR_%s_CELL_%.4d',...
                            Name, FieldNames{j}, k)}];
                    end
                else
                    MatrixList=[MatrixList;{TempMat}];
                    AliasList=[AliasList;{sprintf('%s_VAR_%s', Name, FieldNames{j})}];
                end
            end
        end
    end
